...
export function updateFocussedStateOnMutation(
    ...
    // Fully re-initialize overlay with all styles to ensure consistency
    const wasOutlinePreviouslyDisabled = wasFocusOverlayDisabled(focusOverlayWrapper);
    addFocusOverlay(selectedElement, focusOverlayWrapper, wasOutlinePreviouslyDisabled);

    /**
     * Update the focus toolbar if it exists.
     */
    if (focusedToolbar) {
        if (focusedToolbar.children.length === 0 && resizeObserver) {
            if (selectedElementCslp) {
                const fieldMetadata = extractDetailsFromCslp(selectedElementCslp);
                const eventDetails: VisualBuilderCslpEventDetails = {
                    editableElement: selectedElement,
                    cslpData: selectedElementCslp,
                    fieldMetadata,
                };
            ...
            // Toolbar has content, just update position
            positionToolbar({
                focusedToolbar,
                selectedElementDimension,
            });
